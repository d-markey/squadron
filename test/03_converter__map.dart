...
void testLazyMaps(TestContext tc) {
  _MapDataContext<V> $initDataContext<V>(Map data, Cast<V> cast) => (
  ...
  tc.group('- Doubles', () {
    ...
    for (var feature in MapTests._dblTests.entries) {
      final dc = $initDataContext(data, MapTests._dbl);
      feature.test(tc, dc);
    }
  });
  ...
  tc.group('- Nullable doubles', () {
    ...
    for (var feature in MapTests._ndblTests.entries) {
      final dc = $initDataContext(data, MapTests._ndbl);
      feature.test(tc, dc);
    }
  });
  ...
  tc.group('- Nullable integers', () {
    ...
    for (var feature in MapTests._nintTests.entries) {
      final dc = $initDataContext(data, MapTests._nint);
      feature.test(tc, dc);
    }
  });
}
...
class MapTests {
// doubles
  static final _dx = {'1dot2': 1.2, '1dot3': 1.3, '1dot4': 1.4, '1dot5': 1.5};

  static final _dblFeats = _MapFeatures<double>();
  static final _dblTests = <String, _MapFeatureTest<double>>{
    ...
    'set key (new)': (m) =>
        _dblFeats.setFor(m, _dx.keys.first, _dx.values.first),
    ...
    'addEntries (empty)': (m) =>
        _dblFeats.addEntries(m, _Map<double>().entries),
    'putIfAbsent': (m) => _dblFeats.putIfAbsent(m, _dx.keys.first, _nf),
    'putIfAbsent (existing)': (m) =>
        _dblFeats.putIfAbsent(m, m.keys.first, _nf),
    'update': (m) => _dblFeats.update(m, m.keys.first, _dblSq, _nf),
    'update (no default)': (m) =>
        _dblFeats.update(m, m.keys.first, _dblSq, null),
    ...
    'removeWhere (key)': (m) =>
        _dblFeats.removeWhere(m, (k, v) => k.length > 4),
  ...
  static final _ndblFeats = _MapFeatures<double?>();
  static final _ndblTests = <String, _MapFeatureTest<double?>>{
    ...
    'set key': (m) =>
        _ndblFeats.setFor(m, m.keys.first, _dblSq(m.values.first)),
    'set key (new)': (m) =>
    ...
    'addEntries (empty)': (m) =>
        _ndblFeats.addEntries(m, _Map<double>().entries),
    'putIfAbsent': (m) => _ndblFeats.putIfAbsent(m, _dx.keys.first, _nf),
    'putIfAbsent (existing)': (m) =>
        _ndblFeats.putIfAbsent(m, m.keys.first, _nf),
    ...
    'removeWhere (key)': (m) =>
        _ndblFeats.removeWhere(m, (k, v) => k.length > 4),
    'removeWhere (value)': (m) =>
        _ndblFeats.removeWhere(m, (k, v) => _isGt3(v)),
...
// ints
  ...
  static final _nintFeats = _MapFeatures<int?>();
  static final _nintTests = <String, _MapFeatureTest<int?>>{
    ...
    'set key': (m) =>
        _nintFeats.setFor(m, m.keys.first, _intSq(m.values.first)),
    'set key (new)': (m) =>
    ...
    'putIfAbsent (existing)': (m) =>
        _nintFeats.putIfAbsent(m, m.keys.first, _nf),
    ...
    'removeWhere (key)': (m) =>
        _nintFeats.removeWhere(m, (k, v) => k.length > 4),
    'removeWhere (value)': (m) =>
        _nintFeats.removeWhere(m, (k, v) => _isGt3(v)),
...
// helpers
  ...
  static int _intSq(num? n) => _int(n ?? 0) * _int(n ?? 0);
  static double _dblSq(num? n) => _dbl(n ?? 0) * _dbl(n ?? 0);

  static double? _neg(num? n) => (n == null) ? null : -_dbl(n);

  static T _nf<T>() => -1 as T;
}

typedef _Map<V> = Map<String, V>;
typedef _MapDataContext<V> = ({LazyInPlaceMap<String, V> lazy, _Map<V> ref});
typedef _MapFeatureTest<V> = dynamic Function(_Map<V>);

typedef _MapAction<V> = void Function(String, V);
typedef _MapDefault<V> = V Function();
typedef _MapMapper<V, U> = U Function(String, V);
typedef _MapUpdater<V> = V Function(V);

class _MapFeatures<V> {
  ...
  int forEach(_Map<V> m, _MapAction<V> a) {
    int n = 0;
    void $a(String k, V v) {
      a(k, v);
      n++;
  ...
  Map<K2, V2> map<K2, V2>(_Map<V> m, _MapMapper<V, MapEntry<K2, V2>> c) =>
      m.map(c);

  V putIfAbsent(_Map<V> m, String k, _MapDefault<V> d) => m.putIfAbsent(k, d);

  V? remove(_Map<V> m, Object? key) => m.remove(key);

  _Map<V> removeWhere(_Map<V> m, _MapMapper<V, bool> t) => m..removeWhere(t);

  String getString(_Map<V> m) => m.toString();

  V update(_Map<V> m, String k, _MapUpdater<V> u, _MapDefault<V>? d) =>
      m.update(k, u, ifAbsent: d);

  _Map<V> updateAll(_Map<V> m, _MapMapper<V, V> u) => m..updateAll(u);
}

extension _MapFeatureTestExt<V> on MapEntry<String, _MapFeatureTest<V>> {
  void test(TestContext tc, _MapDataContext<V> data) {
    tc.test('- $key', () {
