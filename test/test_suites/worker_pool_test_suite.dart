  ...
  group("- WorkerPool", () {
    ...
    group('- initialization error', () {
      test('- failed init', () async {
        final pool = TestWorkerPool.throws();
        try {
          try {
            await pool.start();
      ...
      test('- invalid command ID', () async {
        final pool = TestWorkerPool.invalid();
        try {
          try {
            await pool.start();
            throw Exception('start() returned successfully');
    ...
    group('- error handling', () {
      test('- Exception', () async {
        ...
        try {
          ...
          expect(
              pool.stats.fold<int>(0, (p, s) => p + s.totalErrors), equals(1));
        } finally {
          pool.stop();
        }
      });
      ...
      test('- WorkerException', () async {
        ...
        try {
          try {
            ...
            expect(ex.stackTrace?.toString(), contains('throwWorkerException'));
            expect(ex.workerId, isNotNull);
          }
          expect(
              pool.stats.fold<int>(0, (p, s) => p + s.totalErrors), equals(1));
      ...
      test('- CustomException - after deserializer deregistation', () async {
        ...
        Squadron.exceptionManager?.unregister(CustomException.typeId);
