  ...
  group("- Worker", () {
    ...
    group('- start/stop', () {
      ...
      test('- hook', () async {
        ...
        await worker.start();
        expect(
            platformTypeName, equals(TestContext.isJs ? 'Worker' : 'Isolate'));
      ...
      group('- Install', () {
        test('No error', () async {
          ...
          expect(
              memoryLogger.contains('service installed successfully'), isTrue);
          ...
          worker.stop();
          expect(
              memoryLogger.contains('intended failure on uninstall'), isFalse);
          expect(memoryLogger.contains('service uninstalled successfully'),
              isFalse);
        ...
        test('Error on installation', () async {
          ...
          try {
            ...
            expect(
                memoryLogger.contains('intended failure on install'), isTrue);
            expect(memoryLogger.contains('service installed successfully'),
                isFalse);
          }
    ...
    group('- initialization error', () {
      test('- failed init', () async {
        final worker = TestWorker.throws();
        try {
          await worker.start();
      ...
      test('- invalid command ID', () async {
        final worker = TestWorker.invalid();
        try {
          await worker.start();
          throw Exception('start() returned successfully');
        } catch (ex) {
    ...
    group('- error handling', () {
      ...
      test('- TaskTimeOutException', () async {
        ...
        try {
          ...
          final wex = ex as SquadronTimeoutException;
      ...
      test('- CanceledException', () async {
        ...
        try {
          ...
          expect(ex, isA<SquadronCanceledException>());
          final wex = ex as SquadronCanceledException;
          expect(wex.message, contains('intentional canceled exception'));
        }
        expect(worker.stats.totalErrors, equals(errors + 1));
      });

      test('- CustomException', () async {
        Squadron.exceptionManager
