...
void testLazyInPlaceCastConverter(TestContext tc) {
  tc.group('- LazyInPlaceConverter + CastConverter', () {
    final converter = LazyInPlaceConverter(CastConverter.instance);
    ...
    tc.group('- lists', () {
      ...
      tc.test('- doubles + int', () async {
        ...
        // accessing valid items work
        expect(data[0], isA<double>());
        expect(res[0], data[0]);

        try {
          // accessing "invalid" items throws if non-JS
          expect(data[2], isA<int>());
          expect(res[2], data[2]);
          _unexpectedSuccessIfNonJs('int to double', res[2]);
        } on TypeError catch (ex) {
    ...
    tc.group('- maps', () {
      ...
      tc.test('- String / ints + integral double (cast)', () async {
        ...
        // accessing valid item works
        expect(data['one'], isA<int>());
        expect(res['one'], data['one']);

        try {
          // accessing "invalid" item throws if non-JS
          expect(data['two'], isA<double>());
          expect(res['two'], data['two']);
          _unexpectedSuccessIfNonJs('integral double to int', res['two']);
        } on TypeError catch (ex) {
          _unexpectedFailureIfJs('integral double to int', ex);
        }
