...
void testLazyLists(TestContext tc) {
  _ListDataContext<T> $initDataContext<T>(List data, Cast<T> cast) =>
  ...
  tc.group('- Doubles', () {
    ...
    for (var feature in ListTests._dblTests.entries) {
      final dc = $initDataContext(data, ListTests._dbl);
      feature.check(tc, dc);
    }
  });
  ...
  tc.group('- Nullable doubles', () {
    ...
    for (var feature in ListTests._ndblTests.entries) {
      final dc = $initDataContext(data, ListTests._ndbl);
      feature.check(tc, dc);
    }
  });
  ...
  tc.group('- Nullable integers', () {
    ...
    for (var feature in ListTests._nintTests.entries) {
      final dc = $initDataContext(data, ListTests._nint);
      feature.check(tc, dc);
    }
  });
}
...
class ListTests {
// doubles
  static final _dx = [1.2, 1.3, 1.4, 1.5];

  static final _dblFeats = _ListFeatures<double>();
  static final _dblTests = <String, _ListFeatureTest<double>>{
    ...
    'singleWhere (match)': (l) =>
        _dblFeats.singleWhere(l, (n) => n == 2.2, _nf),
    ...
    'fillRange (invalid)': (l) =>
        _dblFeats.fillRange(l, -1, l.length, _dx.first),
  ...
  static final _ndblFeats = _ListFeatures<double?>();
  static final _ndblTests = <String, _ListFeatureTest<double?>>{
    ...
    'subList (end to end)': (l) =>
        _ndblFeats.sublist(l, l.length - 1, l.length),
    ...
    'singleWhere (match)': (l) =>
        _ndblFeats.singleWhere(l, (n) => n == 2.2, _nf),
...
// ints
  static final _ix = [12, 13, 14, 15];

  static final _nintFeats = _ListFeatures<int?>();
  static final _nintTests = <String, _ListFeatureTest<int?>>{
    ...
    'subList (end to end)': (l) =>
        _nintFeats.sublist(l, l.length - 1, l.length),
...
// helpers
  static final _seed = DateTime.now().millisecondsSinceEpoch ~/ 1000;

  static final _int = NumConverter.instance.value<int>();
  static final _nint = Converter.allowNull(_int);
  static final _dbl = NumConverter.instance.value<double>();
  static final _ndbl = Converter.allowNull(_dbl);

  static bool _isNum(num? n) => n is num;
  static bool _isGt3(num? n) => (n != null) && (_dbl(n) > 3);
  static bool _isGt4(num? n) => (n != null) && (_dbl(n) > 4);
  static bool _isInt(num? n) =>
      _isNum(n) && (_dbl(n).toInt().toDouble() == _dbl(n));

  static bool _isNeg(num? n) => (n != null) && (_dbl(n) < 0);

  static T _nf<T>() => -1 as T;
}

typedef _ListAction<T> = void Function(T);
typedef _ListDataContext<T> = ({LazyInPlaceList<T> lazy, List<T> ref});
typedef _ListDefault<T> = T Function();
typedef _ListFeatureTest<T> = dynamic Function(List<T>);
typedef _ListMapper<T, U> = U Function(T);
typedef _ListReducer<U, V, X> = X Function(U, V);

class _ListFeatures<T> {
  List<T> add(List<T> l, T v) => l..add(v);

  List<T> addAll(List<T> l, Iterable<T> x) => l..addAll(x);

  bool any(List<T> l, _ListMapper<T, bool> t) => l.any(t);
  ...
  bool every(List<T> l, _ListMapper<T, bool> t) => l.every(t);

  Iterable<U> expand<U>(List<T> l, _ListMapper<T, Iterable<U>> e) =>
      l.expand(e);

  void fillRange(List<T> l, int s, int e, T? v) => l.fillRange(s, e, v);

  T firstWhere(List<T> l, _ListMapper<T, bool> t, _ListDefault<T>? d) =>
      l.firstWhere(t, orElse: d);

  U fold<U>(List<T> l, U v, _ListReducer<U, T, U> f) => l.fold(v, f);

  Iterable<T> followedBy(List<T> l, Iterable<T> x) => l.followedBy(x);

  int forEach(List<T> l, _ListAction<T> a) {
    var n = 0;
    void $a(T i) {
      a(i);
      n++;
  ...
  int indexWhere(List<T> l, _ListMapper<T, bool> t, int? s) =>
      l.indexWhere(t, s ?? 0);
  ...
  int lastIndexWhere(List<T> l, _ListMapper<T, bool> t, int? s) =>
      l.lastIndexWhere(t, s);

  T lastWhere(List<T> l, _ListMapper<T, bool> t, _ListDefault<T>? d) =>
      l.lastWhere(t, orElse: d);

  Iterable<U> map<U>(List<T> l, _ListMapper<T, U> e) => l.map(e);

  T reduce(List<T> l, _ListReducer<T, T, T> c) => l.reduce(c);
  ...
  List<T> removeWhere(List<T> l, _ListMapper<T, bool> t) => l..removeWhere(t);

  List<T> replaceRange(List<T> l, int s, int e, Iterable<T> r) =>
      l..replaceRange(s, e, r);

  List<T> retainWhere(List<T> l, _ListMapper<T, bool> t) => l..retainWhere(t);
  ...
  T singleWhere(List<T> l, _ListMapper<T, bool> t, _ListDefault<T>? d) =>
      l.singleWhere(t, orElse: d);

  Iterable<T> skip(List<T> l, int c) => l.skip(c);

  Iterable<T> skipWhile(List<T> l, _ListMapper<T, bool> t) => l.skipWhile(t);

  List<T> sort(List<T> l, _ListReducer<T, T, int>? c) => l..sort(c);

  List<T> sublist(List<T> l, int s, int? e) => l.sublist(s, e);

  Iterable<T> take(List<T> l, int c) => l.take(c);

  Iterable<T> takeWhile(List<T> l, _ListMapper<T, bool> t) => l.takeWhile(t);

  List<T> toList(List<T> l) => l.toList();

  Set<T> toSet(List<T> l) => l.toSet();

  Iterable<T> where(List<T> l, _ListMapper<T, bool> t) => l.where(t);

  Iterable<U> whereType<U>(List<T> l) => l.whereType<U>();
}

extension _ListFeatureTestExt<T> on MapEntry<String, _ListFeatureTest<T>> {
  void check(TestContext tc, _ListDataContext<T> data) {
    tc.test('- $key', () {
