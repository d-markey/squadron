    ...
    tc.group('- CANCELATION', () {
      tc.group('- ValueTask', () {
        tc.test(
            ...
            () => TestWorkerPool(tc, small).startAndRunTest((p) async {
                  final count = 2 * p.maxConcurrency + 1;
                  final tasks = <Future>[], digits = <int>[];
                  int errors = 0;
                  for (var i = 0; i < count; i++) {
                    tasks.add(p.delayedLong(i).then(
                          digits.add,
                          onError: (_) => errors++,
                        ));
                  }
            ...
            () => TestWorkerPool(tc, small).startAndRunTest((p) async {
                  ...
                  for (var i = 0; i < count; i++) {
                    tasks.add(
                      p
                          .delayedLong(i)
                          .then(digits.add, onError: (e) => errors++),
                    );
                  }
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc, small).startAndRunTest((p) async {
                  ...
                  for (var i = 0; i < count; i++) {
                    ...
                    tasks.add(task);
                    futures.add(
                        task.value.then(digits.add, onError: (_) => errors++));
                  }
                  ...
                  expect(tasks.where((t) => t.isFinished),
                      hasLength(count - errors));
                }));
      ...
      tc.group('- StreamTask', () {
        tc.test(
            ...
            () => TestWorkerPool(tc, small).startAndRunTest((p) async {
                  ...
                  expect(
                      errors, everyElement(reports('Immediate cancelation')));
                }));
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc, small).startAndRunTest((p) async {
                  ...
                  for (var i = 0; i < count; i++) {
                    ...
                    // finite emits one number per tick, up to the specified upper bound
                    // task #1 (i = 0) will emit 1 number in 1 tick, task #2 3 numbers in 3 ticks, task #3 5 numbers in 5 ticks...
                    p.finite(2 * i + 1).listen(res.add,
                        onError: res.add, onDone: completer.complete);
                  }

                  await Future.delayed(TestDelay.tick * 4);
                  // first and second task should have completed
                  p.cancelAll();
                  ...
                  expect(fullExecutions + partialExecutions + fullCancelations,
                      count);
                }));
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc, small).startAndRunTest((p) async {
                  ...
                  expect(status.where((s) => s == 'started'),
                      hasLength(isPositive));
      ...
      tc.group('- CancelationToken', () {
        tc.test(
            ...
            () => TestWorker(tc).startAndRunTest((w) async {
                  ...
                  Timer(TestDelay.tick * N, cancelation.cancel);
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc).startAndRunTest((p) async {
                  ...
                  Timer(TestDelay.tick * N * 2, token.cancel);

                  final res =
                      await _testFinitePoolCancelation(p, N, count, token);
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc).startAndRunTest((p) async {
                  ...
                  final res =
                      await _testInfinitePoolCancelation(p, count, token);
      ...
      tc.group('- TimeoutToken', () {
        tc.test(
            '- Finite() worker',
            () => TestWorker(tc).startAndRunTest((w) async {
                  final N = 15, timeout = TimeoutToken(TestDelay.tick * N);
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc).startAndRunTest((p) async {
                  ...
                  final timeout = TimeoutToken(TestDelay.tick * N * 1.9);

                  final res =
                      await _testFinitePoolCancelation(p, N, count, timeout);
                  expect(timeout.isCanceled, isTrue);
                  expect(p.pendingWorkload, isZero);
                  expect(res.success, lessThanOrEqualTo(p.maxConcurrency));
                  expect(res.errors, count - res.success);
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc).startAndRunTest((p) async {
                  ...
                  final res =
                      await _testInfinitePoolCancelation(p, count, timeout);
                  expect(timeout.isCanceled, isTrue);
                  expect(p.pendingWorkload, isZero);
                  expect(res.success, 0);
                  expect(res.errors, count);
      ...
      tc.group('- CompositeToken', () {
        tc.test(
            ...
            () => TestWorker(tc).startAndRunTest((w) async {
                  ...
                  final timeout1 = TimeoutToken(TestDelay.tick * N * 1.9);
                  final cancelation1 = CancelableToken();
                  Timer(timeout1.timeout * 0.25, cancelation1.cancel);
                  final composite1 =
                      CompositeToken.any([timeout1, cancelation1]);
                  expect(composite1.isCanceled, isFalse);
                  // ensure timeout is stopped when composite is cancelled
                  composite1.onCanceled.whenComplete(timeout1.stop);
                  ...
                  final timeout2 = TimeoutToken(TestDelay.tick * N * 1.9);
                  final cancelation2 = CancelableToken();
                  Timer(timeout2.timeout * 2, cancelation2.cancel);
                  final composite2 =
                      CompositeToken.any([timeout2, cancelation2]);
                  expect(composite2.isCanceled, isFalse);
                  // ensure timeout is stopped when composite is cancelled
                  composite2.onCanceled.whenComplete(timeout2.stop);
                  ...
                  final timeout3 = TimeoutToken(TestDelay.tick * N * 1.9);
                  final cancelation3 = CancelableToken();
                  Timer(timeout3.timeout * 0.25, cancelation3.cancel);
                  final composite3 =
                      CompositeToken.all([timeout3, cancelation3]);
                  // ensure timeout is stopped when composite is cancelled
                  composite3.onCanceled.whenComplete(timeout3.stop);
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc).startAndRunTest((p) async {
                  ...
                  // canceled by cancelation1
                  final timeout1 = TimeoutToken(TestDelay.tick * N * 2);
                  final cancelation1 = CancelableToken();
                  Timer(timeout1.timeout * 0.25, cancelation1.cancel);
                  final composite1 =
                      CompositeToken.any([timeout1, cancelation1]);
                  ...
                  var res =
                      await _testFinitePoolCancelation(p, N, count, composite1);
                  expect(composite1.isCanceled, isTrue);
                  expect(timeout1.isCanceled, isFalse);
                  expect(cancelation1.isCanceled, isTrue);
                  expect(p.pendingWorkload, isZero);
                  ...
                  // canceled by timeout2
                  final timeout2 = TimeoutToken(TestDelay.tick * N * 2);
                  final cancelation2 = CancelableToken();
                  ...
                  res =
                      await _testFinitePoolCancelation(p, N, count, composite2);
                  expect(composite2.isCanceled, isTrue);
                  expect(timeout2.isCanceled, isTrue);
                  expect(cancelation2.isCanceled, isFalse);
                  expect(p.pendingWorkload, isZero);
                  ...
                  // canceled by timeout3 and cancelation3
                  final timeout3 = TimeoutToken(TestDelay.tick * N * 2);
                  final cancelation3 = CancelableToken();
                  Timer(timeout3.timeout * 0.25, cancelation3.cancel);
                  final composite3 =
                      CompositeToken.all([timeout3, cancelation3]);
                  ...
                  res =
                      await _testFinitePoolCancelation(p, N, count, composite3);
                  expect(composite3.isCanceled, isTrue);
                  expect(timeout3.isCanceled, isTrue);
                  expect(cancelation3.isCanceled, isTrue);
                  expect(p.pendingWorkload, isZero);
        ...
        tc.test(
            ...
            () => TestWorkerPool(tc).startAndRunTest((p) async {
                  ...
                  var res =
                      await _testInfinitePoolCancelation(p, count, composite1);
                  expect(composite1.isCanceled, isTrue);
                  expect(timeout1.isCanceled, isFalse);
                  expect(cancelation1.isCanceled, isTrue);
                  expect(p.pendingWorkload, isZero);
                  ...
                  res =
                      await _testInfinitePoolCancelation(p, count, composite2);
                  expect(composite2.isCanceled, isTrue);
                  expect(timeout2.isCanceled, isTrue);
                  expect(cancelation2.isCanceled, isFalse);
                  expect(p.pendingWorkload, isZero);
                  ...
                  res =
                      await _testInfinitePoolCancelation(p, count, composite3);
                  expect(composite3.isCanceled, isTrue);
                  expect(timeout3.isCanceled, isTrue);
                  expect(cancelation3.isCanceled, isTrue);
                  expect(p.pendingWorkload, isZero);
...
Future<({List<int> digits, Object? exception})> _testFiniteCancelation(
  ...
  final digits = <int>[];
  try {
    await worker.finite(50 * N, token).map(digits.add).toList();
  } catch (ex) {
    exception = ex;
  }
...
Future<({List<int> digits, Object? exception})> _testInfiniteCancelation(
  ...
  final digits = <int>[];
  try {
    await worker.infinite(token).map(digits.add).toList();
  } catch (ex) {
    exception = ex;
  }
  return (digits: digits, exception: exception);
